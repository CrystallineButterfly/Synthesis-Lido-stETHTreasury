...
contract YieldWindowTreasuryTest is Test {
    YieldWindowTreasury private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_lido_stethtreasury");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new YieldWindowTreasury(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        YieldWindowTreasury freshController = new YieldWindowTreasury(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
